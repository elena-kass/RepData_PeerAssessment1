...
```{r Code for creating histogram plot and calculating mean and median}
library(dplyr)
...
```{r Code to plot the average daily activity pattern }
...
``` {r Code to identify the 5-minute interval which on average across all the days contains the maximum number of steps, results = "hide"}

max_interval <- mean_interval[which.max(mean_interval$steps),]
```
### The interval with the daily maximum number of steps is `r max_interval$interval`.
...
```
### There are `r total_NA` rows containing NA values in our dataframe.
...
```{r Code to impute the NA values, results = "hide"}
noNAdf <- df
...
```
### The mean number of steps taken each day is `r noNAmeansteps` and the median is `r noNAmediansteps`.
...
```{r Code to label each day as weekend or weekday, results = "hide"}
for (i in 1:nrow(noNAdf)) {
...
```{r Code to create plot comparing weekend versus weekday}
library(ggplot2)
